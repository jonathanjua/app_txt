...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Escape' && driveModal && !driveModal.classList.contains('hidden')) hideDriveModal();
});

// --- Modal Google Drive ---
const driveModal = document.getElementById('drive-modal');
const driveModalClose = document.getElementById('drive-modal-close');
const driveStatus = document.getElementById('drive-status');
const driveCredentialsSection = document.getElementById('drive-credentials-section');
const driveConnectBtn = document.getElementById('drive-connect');
const driveUploadBtn = document.getElementById('drive-upload');
const driveOpenBtn = document.getElementById('drive-open');
const driveClientId = document.getElementById('drive-client-id');
const driveClientSecret = document.getElementById('drive-client-secret');
const driveSaveCredentialsBtn = document.getElementById('drive-save-credentials');
...
async function refreshDriveStatus() {
  ...
  driveCredentialsSection.classList.add('hidden');
  try {
    const status = await window.api.drive.getStatus();
    if (!status.hasCredentials) {
      driveStatus.textContent = 'Configure as credenciais OAuth do Google Cloud para usar o Drive.';
      driveCredentialsSection.classList.remove('hidden');
      driveConnectBtn.disabled = true;
      return;
    }
    if (!status.hasTokens) {
      driveStatus.textContent = 'Não conectado. Clique em "Conectar ao Google Drive".';
      driveConnectBtn.disabled = false;
      return;
    }
    driveStatus.textContent = 'Conectado ao Google Drive.';
    driveConnectBtn.textContent = 'Reconectar';
    driveConnectBtn.disabled = false;
    driveUploadBtn.disabled = false;
    driveOpenBtn.disabled = false;
  } catch (e) {
    driveStatus.textContent = 'Erro: ' + (e.message || 'Desconhecido');
  }
}

driveModalClose?.addEventListener('click', hideDriveModal);
driveModal?.addEventListener('click', (e) => {
  if (e.target === driveModal) hideDriveModal();
});

driveSaveCredentialsBtn?.addEventListener('click', async () => {
  const id = driveClientId?.value?.trim();
  const secret = driveClientSecret?.value?.trim();
  if (!id || !secret) {
    showStatus('Preencha Client ID e Client Secret', 3000);
    return;
  }
  try {
    await window.api.drive.saveCredentials(id, secret);
    showStatus('Credenciais salvas');
    refreshDriveStatus();
  } catch (e) {
    showStatus('Erro ao salvar: ' + (e.message || ''), 4000);
  }
});

driveConnectBtn?.addEventListener('click', async () => {
  driveConnectBtn.disabled = true;
  driveStatus.textContent = 'Abrindo login do Google...';
  try {
    ...
    driveStatus.textContent = 'Erro: ' + (e.message || 'Falha ao conectar');
    showStatus(e.message || 'Falha ao conectar', 4000);
  } finally {
    driveConnectBtn.disabled = false;
  }
});
...
driveUploadBtn?.addEventListener('click', async () => {
  const tab = getCurrentTab();
  if (!tab) return;
  const name = getTabLabel(tab.path) || 'documento.txt';
  driveUploadBtn.disabled = true;
  driveStatus.textContent = 'Enviando...';
  try {
    await window.api.drive.upload(name, editor.value);
    driveStatus.textContent = 'Arquivo enviado ao Drive.';
    showStatus('Enviado ao Google Drive');
  } catch (e) {
    driveStatus.textContent = 'Erro: ' + (e.message || 'Falha no envio');
    showStatus(e.message || 'Falha no envio', 4000);
  } finally {
    driveUploadBtn.disabled = false;
  }
});
...
driveOpenBtn?.addEventListener('click', async () => {
  let list = [];
  driveOpenBtn.disabled = true;
  driveStatus.textContent = 'Listando arquivos...';
  try {
    list = await window.api.drive.list();
  } catch (e) {
    driveStatus.textContent = 'Erro: ' + (e.message || 'Falha ao listar');
    driveOpenBtn.disabled = false;
    return;
  }
  if (list.length === 0) {
    driveStatus.textContent = 'Nenhum arquivo de texto no Drive.';
    driveOpenBtn.disabled = false;
    return;
  }
  driveStatus.textContent = 'Selecione um arquivo na lista (em breve: diálogo). Abrindo o mais recente.';
  const latest = list[0];
  try {
    const content = await window.api.drive.download(latest.id);
    createNewTab(latest.name, content);
    renderTabBar();
    updateStatus();
    showStatus('Aberto do Drive: ' + latest.name);
    driveStatus.textContent = 'Conectado ao Google Drive.';
  } catch (e) {
    driveStatus.textContent = 'Erro ao abrir: ' + (e.message || '');
    showStatus(e.message || 'Falha ao abrir', 4000);
  } finally {
    driveOpenBtn.disabled = false;
  }
});

document.getElementById('btn-drive')?.addEventListener('click', showDriveModal);
